...
long long clockWiseMinAns(vector<int> products){
    int n = products.size();
    ...
    while(i<n && j<n){
        ...
        if(i<n && j<n){
            ...
            long long amount = min(extra, remaining);
...
long long getMinRedistributionCost(vector<int>& products){
    ...
    long long clockWiseAns = clockWiseMinAns(products);

    vector<int> reversed = products;
    
    reverse(reversed.begin(),reversed.end());

    long long antiClockwiseAns = clockWiseMinAns(reversed);


    return min(clockWiseAns,antiClockwiseAns);
